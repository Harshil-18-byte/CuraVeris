...
from app.db.models import Bill, DisputeLetter
...
async def generate_dispute_letter(req: DisputeLetterRequest, db: AsyncSession = Depends(get_db)):
    """
    ...
    letter_obj = DisputeLetter(
        bill_id=bill.id,
        forum_type=letter_data["forum_type"],
        target_authority=letter_data["target_authority"],
        letter_title=letter_data["letter_title"],
        letter_body=letter_data["letter_body"],
        statutory_citations=letter_data["statutory_citations"],
        total_disputed_amount=letter_data["total_disputed_amount"]
    )
    db.add(letter_obj)
    await db.commit()
    ...
    return DisputeLetterResponse(
        ...
        bill_id=letter_obj.bill_id,
        forum_type=letter_obj.forum_type,
        target_authority=letter_obj.target_authority,
        letter_title=letter_obj.letter_title,
        letter_body=letter_obj.letter_body,
        statutory_citations=letter_obj.statutory_citations,
        total_disputed_amount=letter_obj.total_disputed_amount,
        created_at=letter_obj.created_at
    )


@router.get("/docx/{letter_id}")
async def download_dispute_docx(letter_id: str, db: AsyncSession = Depends(get_db)):
